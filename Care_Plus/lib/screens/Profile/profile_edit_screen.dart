...
class _ProfileEditScreenState extends State<ProfileEditScreen> {
    ...
    _docRef.get().then((snap) {
      if (snap.exists) {
        ...
        setState(() {
          ...
          _otherInsuranceController.text   = data['otherInsurance'] ?? _otherInsuranceController.text;
          _familyDoctorController.text     = data['familyDoctor'] ?? _familyDoctorController.text;
          _emergencyContactController.text = data['emergencyContact'] ?? _emergencyContactController.text;
          _medicalController.text          = data['medicalHistory'] ?? _medicalController.text;
          _allergyController.text          = data['allergyHistory'] ?? _allergyController.text;
      ...
      builder: (ctx, child) => Theme(
        data: Theme.of(ctx).copyWith(
          colorScheme: const ColorScheme.light(
            primary: Color(0xFF4CAF50), onPrimary: Colors.white, onSurface: Colors.black,
          ),
        ), child: child!),
    );
    if (picked != null) {
          ...
          children: [
            CircleAvatar(
              ...
              backgroundImage: _avatarImageProvider ??
                (_avatarFile != null ? FileImage(_avatarFile!)
  ...
  Widget _buildCard(Widget child) => Card(
    margin: const EdgeInsets.symmetric(vertical: 8), elevation: 3,
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            _buildCard(ProfileTextBox(controller: _nameController, label: "Name", validator: (v) => v!.isEmpty ? "Required" : null)),
            _buildCard(ProfileTextBox(controller: _dobController, label: "Date of Birth", readOnly: true, onTap: _pickDOB, validator: (v) => v!.isEmpty ? "Required" : null)),
            _buildCard(ProfileTextBox(controller: _ageController, label: "Age", readOnly: true)),
            _buildCard(ProfileTextBox(controller: _emailController, label: "Email", validator: (v) => v!.contains('@') ? null : "Invalid email")),
            _buildCard(ProfileTextBox(controller: _phoneController, label: "Phone", keyboardType: TextInputType.phone, validator: (v) => v!.length < 10 ? "Invalid number" : null)),
            if (!isGuardian) _buildCard(ProfileTextBox(controller: _icController, label: "IC/Passport", validator: (v) => v!.isEmpty ? "Required" : null)),
            _buildCard(
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text("Insurance Plan", style: TextStyle(color: Colors.green.shade600, fontSize: 16)),
                  ...
                  if (_selectedInsurance == 'Others') ...[
                    ...
                    ProfileTextBox(controller: _otherInsuranceController, label: "Specify Insurance", validator: (v) => v!.isEmpty ? "Required" : null),
                  ],
                ],
              ),
            ),
            _buildCard(ProfileTextBox(controller: _heightController, label: "Height (cm)", keyboardType: TextInputType.number, validator: (v) => double.tryParse(v!) != null ? null : "Invalid")),
            _buildCard(ProfileTextBox(controller: _weightController, label: "Weight (kg)", keyboardType: TextInputType.number, validator: (v) => double.tryParse(v!) != null ? null : "Invalid")),
